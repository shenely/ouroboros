...
class BehaviorFactory(type):
    ...
    _database = pymongo.MongoClient("localhost", 27017,
                                    document_class=ObjectDict)\
                                    ["ouroboros"]
    
    def __new__(cls, name, memo={}):        
        doc = cls._database["behaviors"].find_one({"name": name})#from database
    ...
    def __init__(self, name, memo={}):
        ...
        self._required_data = {data.name: BehaviorFactory(data.type, memo)
                               for data in self.doc.faces.data.require}
        self._provided_data = {data.name: BehaviorFactory(data.type, memo)
    ...
    def __call__(self, name, *args, **kwargs):
        ...
        for node in self.doc.nodes:
            ...
            behavior = BehaviorFactory(node.type, self._memo)\
                                      (node.name, **{arg.name: arg.value
            ...
            data_subgraph = networkx.relabel_nodes(behavior._data_graph,
                                                   lambda x: (node.name,)+x)
            control_subgraph = networkx.relabel_nodes(behavior._control_graph,
                                                      lambda x: (node.name,)+x)
            
            #Add the subgraphs to their respective graphs
            obj._data_graph = networkx.union(obj._data_graph, data_subgraph)
            obj._control_graph = networkx.union(obj._control_graph, control_subgraph)

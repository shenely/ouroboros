...
def init(ns):
    ...
    with open(ns.filename, "r") as stream:
        ...
        logger.debug("1st pass - populate internals")
...
async def main(pool):
    ...
    tasks = pool[True, None].data.pop(None)
    any(task.gen.send(e)
        for task in tasks)
    ...
    while True:
        ...
        logger.debug("exec time: %.4f", time.time() - T)
